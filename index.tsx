...
// FIX: Import monaco editor types to provide type safety for the dynamically loaded editor.
...
// FIX: Correctly type the monaco object using the imported `Monaco` type to resolve
// the circular reference (`typeof monaco`) and the "cannot find namespace" error.
...
async function addCell(
  ...
  // FIX: Add a guard to ensure monaco is initialized before use. This prevents
  // runtime errors if addCell is called before the editor is ready.
  if (!monaco) {
...
function initializeGenderSwapTool() {
  ...
  imageUploadInput.addEventListener('change', () => {
    ...
    if (file) {
      ...
      const reader = new FileReader();
      reader.onload = (e) => {
        originalImagePreview.src = e.target?.result as string;
    ...
    try {
      ...
      for (const part of response.candidates[0].content.parts) {
        if (part.inlineData) {
          ...
          const mimeType =
            part.inlineData.mimeType || 'image/png'; // Default MIME type
          transformedImage.src = `data:${mimeType};base64,${base64ImageBytes}`;
  ...
  downloadButton.addEventListener('click', async () => {
    if (
      ...
      transformedImage.classList.contains('hidden') ||
      !transformedImage.src.startsWith('data:')
    ) {
      return;
    }
    ...
    try {
      ...
      const response = await fetch(transformedImage.src);
      const blob = await response.blob();

      // Create an object URL from the Blob
      const url = URL.createObjectURL(blob);

      // Create a link, set its properties, and trigger the download
      const a = document.createElement('a');
      a.href = url;

      const extension = blob.type.split('/')[1] || 'png';
      a.download = `transformed-image.${extension}`;
      ...
      alert(
        'Could not download the image. Please try again or check the console for errors.',
      );
    }
  });
}
...
limitations under the License.`;
  ...
  // Initialize the new Gender Swap tool
  initializeGenderSwapTool();
